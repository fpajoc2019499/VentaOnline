    ...
    Factura.findOne({_id: params.idFactura}).exec(
        (err, factura) => {
            if(err){
                ...
                return res.status(500).send({ mensaje: "No se puede Eliminar u editar una factura terminada" });
            }else{
                Factura.findByIdAndDelete(params.idFactura,(err, Eliminado)=>{
                if(err) return res.status(500).send({mensaje:"Error en la peticion"});
                if(!Eliminado) return res.status(500).send({mensaje:"No se ha podido cancelar la factura, revise el iD"});
...
function FinalizarFactura(req, res) {
    ...
    Factura.findByIdAndUpdate(params.idFactura, final, { new: true }, (err, productoActualizado) => {
        if (err) return res.status(500).send({ mensaje: 'Error en la peticion' });
        if (!productoActualizado) return res.status(500).send({ mensaje: 'No se a podido finalizar la factura' });
